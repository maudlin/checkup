...
| `CHECKUP_MODE`        | closing verdict (`checkup.sh` + renderer) | `tailored`                               | `tailored` (a repo you own): deploy-gated verdict, a low score exits non-zero to fail CI. `audit` (a repo you don't own / due diligence): informational only, framed as "where to invest", **always exits 0** (never gates). |
